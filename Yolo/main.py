...
parser.add_argument('--crop_no=',type=str)
...
crop_value_list = crop_value.split('to')

print("Augmentation Started")

aug_weight = 0
current_file_name = ''
weights = [0.1, 0.2, 0.1,0.2,0.1,0.1,0.1,0.1]   ####### probability values of each augmentation methods
...
for filename in os.listdir(dir):
    if filename.endswith('.jpg') or filename.endswith('.png'):
        for i in range(0,n):
            ...
            # assert os.path.exists(os.path.join(dir, filename.replace(".jpg",'.txt'))), "File Error " + os.path.join(dir, filename.replace(".jpg",'.txt'))
            try : 
                ...
                print(f'No Label FIle for image {os.path.join(dir, filename.replace(".jpg",".txt"))}')
...
for filename in list_of_files:
    if filename.endswith('.jpg') or filename.endswith('.png'):
        ...
        crop_num = int(random.randrange(float(crop_value_list[0]), float(crop_value_list[1])))
        aug_methods = [                                               ###### Augmentation methods
            ('ori', aug.original, (filename,outdir,i)),
            ('ro', aug.rotate, (filename,angle,outdir,i,type)),
            ...
            ('cro',aug.crop_image, (filename,outdir,crop_num))
        ]
...
rename.change_name(outdir)  ########## change the data name
